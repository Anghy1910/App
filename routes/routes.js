...
router.post('/nuevo-estudiante', (req, res)=>{
  ...
  let nuevoAlumno = 'INSERT INTO actores(nombres, apellidos, correo, documento, telefono_celular, fecha_nacimiento, institucion_id) VALUES(?,?,?,?,?,?,?)';
...
router.put('/estudiante/:id',(req,res)=>{
  ...
  mysqlConnection.query('UPDATE actores SET nombres = ?, apellidos = ?, correo= ?, documento = ?, telefono_celular = ?, fecha_nacimiento = ?, institucion_id = ? WHERE id=?', [nombres, apellidos, correo, documento, telefono_celular, fecha_nacimiento, institucion_id, id], (err, rows, fields)=>{
    if(!err){
...
router.delete('/estudiante/:id', (req, res)=>{
  const {id} = req.params
  mysqlConnection.query('DELETE FROM actores WHERE id=?', [id],(err,rows,fields)=>{
  if(!err){
